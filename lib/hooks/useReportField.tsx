...
export function useReportField({ sectionIdx, subSectionIdx, fieldIdx, field }: Props) {
	...
	const onSetAttributePath = useRecoilCallback(({ set, snapshot }) => async (e: React.MouseEvent) => {
		...
		// Shift多選
			...
			const normalizedFieldPaths = await snapshot.getPromise(normalizedFieldPathsSelector);
			const fieldPathList = findFieldPathsBetween(normalizedFieldPaths, previousPath, path);
